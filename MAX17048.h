...
public:
  ...
  void attatch(TwoWire& w) {
    wire = &w;
  }

  uint16_t adc() {
    return read(REG::VCELL);
  }
  float voltage() {
  ...
  uint8_t percent() {
    return (uint8_t)(read(REG::SOC) / 256);
  }
  float accuratePercent() {
    return (float)read(REG::SOC) / 256.f;
  }
  ...
  bool isSleepable() {
    return bitRead(read(REG::MODE), 13);
  }

  uint16_t mode() {
    return read(REG::MODE);
  }

  uint8_t version() {
    return (uint8_t)read(REG::VERSION);
  }

  bool isHibernating() {
    return bitRead(read(REG::MODE), 12);
  }
  float hibernateActTh() {
    return (float)highByte(read(REG::HIBRT)) * 0.00125;
  }
  float hibernateHibTh() {
    return (float)lowByte(read(REG::HIBRT)) * 0.208;
  }
  ...
  uint8_t emptyAlertThreshold() {
    return 32 - (read(REG::CONFIG) & 0x001F);
  }
  ...
  float vAlertMinThreshold() {
    return highByte(read(REG::VALRT)) * 0.02;
  }
  ...
  float vAlertMaxThreshold() {
    return (read(REG::VALRT) & 0x00FF) * 0.02;
  }
  ...
  float vResetThreshold() {
    return read(REG::VRESET_ID >> 9) * 0.04;
  }
  ...
  bool comparatorEnabled() {
    return bitRead(read(REG::VRESET_ID), 8);
  }
  ...
  uint8_t id() {
    return lowByte(read(REG::VRESET_ID));
  }

  bool vResetAlertEnabled() {
    return bitRead(read(REG::STATUS), 14);
  }
  ...
  uint8_t alertFlags() {
    return highByte(read(REG::STATUS)) & 0x3F;
  }
  ...
  float crate() {
    return (float)read(REG::CRATE) * 0.208f;
  }  // % per hour
  ...
  bool highVoltage() {
    return bitRead(alertFlags(), 1);
  }
  bool lowVoltage() {
    return bitRead(alertFlags(), 2);
  }
  bool resetVoltage() {
    return bitRead(alertFlags(), 3);
  }
  bool lowSOC() {
    return bitRead(alertFlags(), 4);
  }
  bool chnageSOC() {
    return bitRead(alertFlags(), 5);
  }
